...
class ConFNet(torch.nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.rnn = nn.LSTM(input_size=63, hidden_size=64, num_layers=1, batch_first=True)
        self.linear_1 = nn.Linear(6, 64)
        self.linear_2 = nn.Linear(64, 128)
        # self.linear_3 = nn.Linear(128, 256)
        self.cnn1 = nn.Conv1d(1, 8, kernel_size=3, stride=1, padding=1)
        self.cnn2 = nn.Conv1d(8, 64, kernel_size=3, stride=1, padding=1)
        self.cnn3 = nn.Conv1d(1, 64, kernel_size=3, stride=1, padding=1)
        self.cnn4 = nn.Conv1d(64, 128, kernel_size=3, stride=1, padding=1)
        # self.linear_4 = nn.Linear(4096, 256)
        # self.linear_5 = nn.Linear(256, 32)
        self.linear_6 = nn.Linear(1984, 2048)
        self.linear_7 = nn.Linear(2048, 256)
        self.linear_8 = nn.Linear(8192, 256)
        self.linear_9 = nn.Linear(256, output_dim)
        ...
        self.bn6 = nn.BatchNorm1d(num_features=2048)
        ...
        self.brnn = nn.BatchNorm1d(num_features=64)
    ...
    def forward(self, x):
        ...
        # x = x.view(-1, 1*1)
        x1 = self.activation(self.bn1(self.linear_1(x)))
        # x1 = self.activation(self.linear_1(x))
        x1 = self.Dropout(x1)
        ...
        x2 = x2.unsqueeze_(1)
        x4 = self.activation(self.bnc(self.cnn1(x2)))
        x4 = self.MaxPool1D(x4)
        x4 = self.Dropout(x4)
        x5 = self.activation(self.bnc2(self.cnn2(x4)))
        x5 = self.MaxPool1D(x5)
        x5 = self.Dropout(x5)
        x5 = x5.view(-1, 64 * 31)
        # x6 = self.activation(self.linear_6(x5))
        ...
        x7 = self.Dropout(x7)
        x7 = x7.unsqueeze_(1)
        x_c = self.activation(self.bnc3(self.cnn3(x7)))
        x_c = self.MaxPool1D(x_c)
        x_c = self.Dropout(x_c)
        x_c2 = self.activation(self.bnc4(self.cnn4(x_c)))
        x_c2 = self.MaxPool1D(x_c2)
        x_c2 = self.Dropout(x_c2)
        outRNN, h_h = self.rnn(x_c2)
        outRNN = outRNN.permute(0, 2, 1)
        outRNN = self.brnn(outRNN).permute(0, 2, 1)
        x_flatten = outRNN.reshape(-1, 128 * 64)
        x8 = self.activation(self.bn8(self.linear_8(x_flatten)))
        x9 = self.linear_9(x8)
        # x4 = self.linear_4(x3)
