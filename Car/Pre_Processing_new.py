...
def eraseforMargin(transitions_list, marg_tatb=120, marg_dx=25, marg_dy=50, marg_dt=1.4):
    #dx_margin
...
if __name__ == '__main__':
    ...
    all_files = [os.path.join(dir, _path) for _path in all_files]
    Dtrain = []
    for path in all_files:
        data = pickle.load(file=open(path, "rb")).values.tolist()
        Dtrain += data

    Car_Data = pd.DataFrame(Dtrain)
    Car_Data = np.asarray(Car_Data.dropna())
    ...
    save_pickle(features, labels, save_path_train, name='DataExtract')
